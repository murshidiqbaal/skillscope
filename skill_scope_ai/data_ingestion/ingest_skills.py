...
if url == "YOUR_SUPABASE_URL" or key == "YOUR_SUPABASE_KEY":
    print("Warning: Please set SUPABASE_URL and SUPABASE_KEY environment variables.")

supabase: Client = create_client(url, key)

mock_skills = [
    {"name": "Flutter", "category": "Mobile App Development", "demand_score": 92.5, "growth_rate": 15.0},
...
def main():
    ...
    for skill in mock_skills:
        try:
            # Check if skill exists to avoid duplicates
            existing = supabase.table("skills").select("id").eq("name", skill["name"]).execute()
            if not existing.data:
                res = supabase.table("skills").insert(skill).execute()
    ...
    # 2. Ingest Resources (Requires fetching skill IDs)
    try:
        ...
        for res in mock_resources:
            ...
            if skill_id:
                ...
                if not existing_res.data:
                    ...
                    print(f"Resource already exists: {res['title']}")
    except Exception as e:
